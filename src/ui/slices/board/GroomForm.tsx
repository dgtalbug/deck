...
export function GroomForm(props: {
  ...
  return (
    <Dialog open onClose={props.onReject} label={`groom note: ${props.noteTitle}`}>
      <DialogHead
        title={`Groom: “${props.noteTitle}”`}
        meta={
          <span>
      ...
      <div class="dialog-actions">
        <button class="btn btn-primary" onClick={accept} aria-disabled={unanswered.length > 0}>
          <Check size={13} /> Accept → groomed
        </button>
        <button class="btn btn-ghost" onClick={props.onReject}>
          <X size={13} /> Reject — leave note untouched
        </button>
      </div>
