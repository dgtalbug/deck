...
const publish = readFileSync(join(import.meta.dir, '../../src/core/board/sync.ts'), 'utf8');

describe('sync drift fix guidance', () => {
  test('the flush-failure drift hint prescribes re-running deck sync', () => {
    expect(publish).toMatch(/fix: 'inspect the provider operation \(deck sync \/ provider status\), fix the cause, then re-run deck sync/);
  });
